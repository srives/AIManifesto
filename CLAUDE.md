...
Added after two consecutive hostile reviews of the same AI executor (AI-BASIC repo, `PRc.md` and `PRx2.md`, 2026-06-10) failed identically: the executor scoped its own verification and certified itself complete — twice, including once immediately after the governing rules were strengthened. Conclusion: rules that rely on executor judgment do not survive executor blind spots. These gates are structural, like 3.5, and bind **every task that edits more than one file**, automatically, with no plan citation and no human prompting required.
